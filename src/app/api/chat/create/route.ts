...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      pdfImagesExtractionResult = await response
        .json()
...
`
        ...
        const response = await generateText({
          ...
          messages: [
            {
              role: 'user',
              content: pdfImagesExtractionResult.map(img => ({
                type: 'image',
                image: img.base64,
              })),
            },
        ...
        const aiTextToJson: { description: string }[] = JSON.parse(
          response.text,
        )
        describedImages = pdfImagesExtractionResult.map(
          ({ base64, ...restExImg }, idx) => ({
            ...restExImg,
      ...
      pdfTextExtractionResult = mergeTextImagesConent(
        // @ts-ignore
