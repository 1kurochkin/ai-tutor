...
const PDFDocument = dynamic(
  () => import('react-pdf').then(mod => ({ default: mod.Document })),
  {
    ssr: false,
    loading: () => (
      <div className="animate-pulse bg-gray-300 h-96 rounded-md" />
    ),
  },
)
...
const PdfView = (props: PdfViewProps) => {
  ...
  useEffect(() => {
    ...
    const applyAnnotations = async () => {
      ...
      try {
        const annotatedPdf = await annotatePDF(pdfBytes, annotations)
        // @ts-ignore
        ...
        if (renderPDFUrl.startsWith('blob:'))
          URL.revokeObjectURL(renderPDFUrl)
  ...
  return (
    <div
      ref={containerRef}
      className={`bg-gray-50 overflow-scroll ${className}`}>
      <PDFDocument
        file={renderPDFUrl}
        // @ts-ignore
